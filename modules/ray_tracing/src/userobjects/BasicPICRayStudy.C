...
registerMooseObject("RayTracingApp", BasicPICRayStudy);

InputParameters
BasicPICRayStudy::validParams()
{
  ...
  // We're going to bank rays manually
  params.set<bool>("_bank_rays_on_completion") = false;

  return params;
}

BasicPICRayStudy::BasicPICRayStudy(const InputParameters & parameters)
...
void
BasicPICRayStudy::postOnSegment(const THREAD_ID tid, const std::shared_ptr<Ray> & ray)
{
  if (tid != 0)
    mooseError("Doesn't support threading yet");

  // Bank the ray if it is done (it hit its max distance)
  // We can then access the rays in _banked_rays on the next timestep
  if (!ray->shouldContinue())
    _banked_rays.push_back(ray);
}

Real
BasicPICRayStudy::maxDistance(const Ray & ray) const
{
  // velocity * dt
