...
void
AQViewFactorRayStudy::getHalfRangeAQ3D(unsigned int cheb_order,
...
void
AQViewFactorRayStudy::getHalfRangeAQ2D(unsigned int leg_order,
                                       std::vector<std::pair<Real, Real>> & x,
                                       std::vector<Real> & w)
{
  // Legendre quadrature
    ...
    _threaded_cached_ray_bcs(libMesh::n_threads())
{
  ...
  if (_mesh.dimension() == 2)
    getHalfRangeAQ2D(getParam<unsigned int>("polar_quad_order"), _aq_angles, _aq_weights);
  else if (_mesh.dimension() == 3)
    getHalfRangeAQ3D(4 * getParam<unsigned int>("azimuthal_quad_order"),
