...
`RayTracingViewFactor` uses the raytracing module to compute view factors for general cavities.
...
computing the integrals in [UnobstructedPlanarViewFactor](UnobstructedPlanarViewFactor.md). The difference
to [UnobstructedPlanarViewFactor](UnobstructedPlanarViewFactor.md) is that the raytracer follows the
...
The `RayTracingViewFactor` requires a `ViewFactorRayStudy` for computing the view factors. The user should
...
by the element id and a *local* face id. As a consequence, a face belongs to a particular element because its
...
Boundaries enclosing a cavity are either external boundary or internal boundaries. External boundaries
...
If element face `(e1, s1)` is just outside of the cavity and `(e2, s2)` is immediately on the other side in the cavity,
...
The [SideSetsBetweenSubdomainsGenerator](SideSetsBetweenSubdomainsGenerator.md) mesh generator can be used for setting up
cavity boundaries. In this mesh generator, the sidesets will belong to the `master` side. Returning to the example,
if `master_block = b1` and `paired_block = b2`, then `(e1,s1)` will be the element faces used in the new sideset.
Specifically, the `master_block` are the blocks around the cavity, while `paired_block` are the blocks inside the cavity.

## Example Input syntax

!listing modules/heat_conduction/test/tests/view_factors/analytical_with_obstruction.i
block=UserObjects
