...
MeshGenerator::validParams()
{
  ...
  params.addParam<bool>("debug_info",
                        false,
                        "Whether or not to print debugging info after generating the mesh "
                        "(bounding box, element types, sidesets, nodesets, subdomains, etc)");
  params.addRangeCheckedParam<unsigned int>("debug_info_verbosity",
                                            2,
                                            "debug_info_verbosity < 2",
                                            "The verbosity flag to pass to MeshBase::get_info() "
                                            "when the parameter \"debug_info\" == true.");
...
MeshGenerator::generateInternal()
{
  ...
  if (getParam<bool>("debug_info"))
    _console << "Debug information for the mesh generated by " << type() << " \"" << name()
             << "\":\n\n"
             << mesh->get_info(2, getParam<unsigned int>("debug_info_verbosity")) << std::endl;
