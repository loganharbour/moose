...
InputParameters
FixedPointExecutor::executorParams()
{
...
InputParameters
FixedPointExecutor::validParams()
{
  InputParameters params = Executor::validParams();
  params += FixedPointExecutor::executorParams();

  params.addRequiredParam<ExecutorName>("executor", "foo");

  return params;
}

FixedPointExecutor::FixedPointExecutor(const InputParameters & parameters)
  : Executor(parameters),
    ...
    _secondary_relaxation_factor(1.0), // will be set by a MultiApp

    _fixed_point_it(0),
    _fixed_point_status(MooseFixedPointConvergenceReason::UNSOLVED),

    _executor(getExecutor("executor")),
...
bool
FixedPointExecutor::solveStep(Real & begin_norm,
                              Real & end_norm,
                              const std::set<dof_id_type> & transformed_dofs)
{
  const bool auto_advance = autoAdvance();

  // Compute previous norms for coloring the norm output
  const Real begin_norm_old =
      (_fixed_point_it > 0 ? _fixed_point_timestep_begin_norm[_fixed_point_it - 1]
                           : std::numeric_limits<Real>::max());
  const Real end_norm_old =
      (_fixed_point_it > 0 ? _fixed_point_timestep_end_norm[_fixed_point_it - 1]
                           : std::numeric_limits<Real>::max());
  ...
  const auto result = _executor.execute();
...
void
FixedPointExecutor::saveAllValues(const bool primary)
{
...
void
FixedPointExecutor::computeCustomConvergencePostprocessor()
{
...
bool
FixedPointExecutor::examineFixedPointConvergence(bool & converged)
{
...
void
FixedPointExecutor::printFixedPointConvergenceReason()
{
...
bool
FixedPointExecutor::autoAdvance() const
{
  // if (dynamic_cast<EigenExecutionerBase *>(&_executioner) && _has_fixed_point_its)
  //   auto_advance = true;

  if (_auto_advance_set_by_user)
    return _auto_advance_user_value;

  return !(_has_fixed_point_its && _problem.isTransient());
}
