...
registerMooseObject("MooseApp", FEProblemSolveExecutor);

std::set<std::string> const FEProblemSolveExecutor::_moose_line_searches = {"contact", "project"};

InputParameters
FEProblemSolveExecutor::executorParams()
{
...
InputParameters
FEProblemSolveExecutor::validParams()
{
  auto params = Executor::validParams();
  params += FEProblemSolveExecutor::executorParams();
  return params;
}

FEProblemSolveExecutor::FEProblemSolveExecutor(const InputParameters & parameters)
  : Executor(parameters),
    ...
    _num_grid_steps(getParam<unsigned int>("num_grids") - 1)
{
  ...
  nl._compute_initial_residual_before_preset_bcs =
      getParam<bool>("compute_initial_residual_before_preset_bcs");
  nl.setVerboseFlag(getParam<bool>("verbose"));
  nl.setDecomposition(getParam<std::vector<std::string>>("splitting"));

  _fe_problem.setSNESMFReuseBase(getParam<bool>("snesmf_reuse_base"),
                                 _pars.isParamSetByUser("snesmf_reuse_base"));
  _fe_problem.skipExceptionCheck(getParam<bool>("skip_exception_check"));
  ...
  nl.offDiagonalsInAutoScaling(getParam<bool>("off_diagonals_in_auto_scaling"));
...
Executor::Result
FEProblemSolveExecutor::run()
{
  Result & result = newResult();

  for (const auto grid_step : make_range(_num_grid_steps + 1))
  {
    _fe_problem.solve();

    if (!_fe_problem.converged())
      result.fail("Failed to converge");

    if (grid_step != _num_grid_steps)
      _fe_problem.uniformRefine();
  }

  if (!_fe_problem.converged())
    result.fail("Failed to converged");
