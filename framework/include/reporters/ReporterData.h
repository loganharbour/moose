                                     ...
                                     const MooseObject & moose_object) const
{
  ...
  RestartableDataValue & restartable_value =
      getRestartableDataHelper(std::move(state_unique_ptr), declare);

  ReporterState<T> * state = dynamic_cast<ReporterState<T> *>(&restartable_value);
...
ReporterData::getReporterValue(const ReporterName & reporter_name,
                               ...
                               const std::size_t time_index /* = 0 */) const
{
  ReporterState<T> & state_ref =
      getReporterStateHelper<T>(reporter_name, /* declare = */ false, consumer);
  state_ref.addConsumer(mode, consumer);
                                   ...
                                   Args &&... args)
{
  ...
  ReporterState<T> & state_ref =
      getReporterStateHelper<T>(reporter_name, /* declare = */ true, producer);
  state_ref.setProducer(producer);
  ...
  if (reporter_name.isPostprocessor() && !state_ref.getReporterName().isPostprocessor())
  {
    state_ref.setIsPostprocessor();
    _states.erase(reporter_name);
    _states.emplace(reporter_name, &state_ref);
  }
  else if (reporter_name.isVectorPostprocessor() &&
           !state_ref.getReporterName().isVectorPostprocessor())
  {
    state_ref.setIsVectorPostprocessor();
    _states.erase(reporter_name);
    _states.emplace(reporter_name, &state_ref);
  }
