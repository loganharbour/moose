...
public:
   ...
   * @param do_exchange Whether or not an exhange is needed, i.e., if all_objects still needs to be
  ...
  SpatiallyClaim(MooseMesh & mesh,
    ...
    : _mesh(mesh),
      ...
      _elem_point_neighbor_map(nullptr),
      _context(context)
  {
  }
  ...
  void claim()
  {
    ...
    _elem_point_neighbor_map = &_mesh.elemPointNeighborMap();
   ...
   * Same as reinit().
   */
  void init() { buildBoundingBoxes(); }

protected:
  /**
   * Entry point for acting on an object before it is attempted to be claimed.
   */
  virtual void prePossiblyClaim(const ObjectType & /* obj */){};
  /**
   * Entry point for acting on an object after it is claimed.
   */
  virtual void postClaim(ObjectType & /* obj */, const Elem * /* elem */){};
  /**
  ...
  {
    mooseAssert(_elem_point_neighbor_map, "Point neighbor map unavailable");
    ...
    const auto * elem = (*_point_locator)(point);
    if (elem)
    {
      ...
      mooseAssert(_elem_point_neighbor_map->count(elem->id()), "Not in point neighbor map");
      const auto & neighbors = _elem_point_neighbor_map->at(elem->id());
      ...
      if (extremum_elem->processor_id() == _pid)
      {
        ...
        postClaim(_local_objects.back(), extremum_elem);
      }
    }
  }

  /**
   * Pushes the data in data (whose key is the processor that each vector of ObjectType that needs
  ...
  const std::unordered_map<dof_id_type, std::vector<const Elem *>> * _elem_point_neighbor_map;
